/*
Originally written in python by admiralGeneralCP5
Converted to C++ by codellama LLM
Edited by admiralGeneralCP5
*/
#include <iostream>
#include <string>
...
int main(int argc, char *argv[]) {
  ...
  // Delete executable after it runs
  cmd = "del " + filename.substr(0, filename.length() - 4);
  status = system(cmd.c_str());

  if (status != 0) {
    std::cout << "Error deleting executable." << std::endl;
    return 1;
  }
  return 0;
}
